...
class TestTheOfflineFlush:
    def test_an_empty_flush_is_not_an_error(self, client: TestClient):
        ...
        payload = client.post("/api/sync/append", json={"ops": []}).json()
        assert payload["stored"] == 0

    def test_a_queue_is_accepted_whole(self, client: TestClient):
        ops = [op(f"o{i}") for i in range(5)]
        payload = client.post("/api/sync/append", json={"ops": ops}).json()
        assert payload["stored"] == 5
        assert payload["head"] >= 5

    def test_flushing_the_same_queue_twice_stores_it_once(self, client: TestClient):
        ...
        ops = [op(f"o{i}") for i in range(5)]
        first = client.post("/api/sync/append", json={"ops": ops}).json()
        second = client.post("/api/sync/append", json={"ops": ops}).json()
    ...
    def test_a_partial_retry_stores_only_what_is_new(self, client: TestClient):
        """The realistic case: the first flush half-landed before the connection died."""
        client.post("/api/sync/append", json={"ops": [op("o0"), op("o1")]})
        payload = client.post(
            "/api/sync/append", json={"ops": [op("o0"), op("o1"), op("o2"), op("o3")]}).json()
        assert payload["stored"] == 2
        assert payload["duplicates"] == 2

    def test_a_malformed_op_is_refused_with_422_not_500(self, client: TestClient):
        """A console on a bad build must get a rejection it can log, not a server error that
        looks like the backend fell over."""
        response = client.post("/api/sync/append", json={"ops": [{"nonsense": True}]})
        assert response.status_code == 422

    def test_one_bad_op_does_not_persist_the_good_ones_beside_it(self, client: TestClient):
        ...
        client.post("/api/sync/append", json={"ops": [op("good"), {"nonsense": True}]})
        after = client.get("/api/health").json()
        assert after.get("head", 0) == before.get("head", 0)


class TestCatchUp:
    def test_a_client_at_zero_receives_everything(self, client: TestClient):
        client.post("/api/sync/append", json={"ops": [op(f"o{i}") for i in range(4)]})
        payload = client.get("/api/sync/since", params={"seq": 0}).json()
        assert len(payload.get("ops", payload.get("entries", []))) == 4

    def test_a_client_already_current_receives_nothing(self, client: TestClient):
        client.post("/api/sync/append", json={"ops": [op(f"o{i}") for i in range(4)]})
...
class TestWorkspacesAreIsolated:
    def test_the_same_op_id_in_two_workspaces_is_not_a_duplicate(self, client: TestClient):
        """Two customers can generate the same client-side id. Treating that as a duplicate would
        silently drop one of their edits."""
        client.post("/api/sync/append", json={"workspace": "a", "ops": [op("same")]})
        payload = client.post(
            "/api/sync/append", json={"workspace": "b", "ops": [op("same")]}).json()
        assert payload["stored"] == 1

    def test_one_workspace_cannot_read_anothers_ops(self, client: TestClient):
        client.post("/api/sync/append", json={"workspace": "a", "ops": [op("secret")]})
